...
"""
From: https://github.com/HazyResearch/state-spaces/blob/137e49d6930414d0b4ca2b0b7e72ef551b1129b0/src/models/sequence/ss/standalone/s4.py
"""
...
class SSKernelNPLR(nn.Module):
    ...
    def _step_state_linear(self, u=None, state=None):
        ...
        if (
            ...
            step_params = {k: _conj(v).to(C.device) for k, v in step_params.items()}
...
if __name__ == "__main__":
    from super_sac import device

    u_conv_seq = torch.randn(3, 100, 20, device=device)
    u_rnn_seq = [torch.randn(3, 20, device=device) for _ in range(50)]
    ...
    model.train()
    model.to(device)
    y_conv, x_conv = model.forward(u_conv_seq)

    model.eval()
    x = model.default_state(3).to(device)
    y = []
    for u_t in u_rnn_seq:
        y_t, x = model.step(u_t, x)
        y.append(y_t)
